...
# 用法: ./run_usbmon_invoke_boottime_full_bus.sh <model_path> <name> <out_dir> <bus>
...
cat "$PW" | sudo -S -p '' sh -c "cat /sys/kernel/debug/usb/usbmon/${BUS}u > '$CAP'" &
CATPID=$!
sleep 0.2
...
while time.time() < deadline and usb_ts is None:
    try:
        with open(cap,'r',errors='ignore') as f:
            lines = f.readlines()
            for ln in reversed(lines[-8000:] if len(lines)>8000 else lines):
                m = pat.match(ln)
                if m:
                    v = float(m.group(1))
